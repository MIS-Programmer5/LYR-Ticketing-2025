                        ...
                        <div class="d-flex mb-9">
                            ...
                            <div class="flex-shrink-0 mr-7 mt-lg-0 mt-3">
                                ...
                                <div class="symbol symbol-lg-75 symbol-primary">
                                    <span class="symbol-label font-size-h3 font-weight-boldest">{{session()->get('user')->name[0]}}</span>
                                </div>
                            </div>
                            <!--end::Pic-->
                            <!--begin::Info-->
                            <div class="flex-grow-1">
                                ...
                                <div class="d-flex justify-content-between flex-wrap mt-1">
                                    <div class="d-flex mr-3">
                                        <a href="#" class="text-dark-75 text-hover-primary font-size-h5 font-weight-bold mr-3">{{session()->get('user')->name}}</a>
                                        <a href="#">
                                    ...
                                    <div class="my-lg-0 my-3">
                                        @if (session()->get("user")->Permission_id==1)
                                            ...
                                            <select onchange="filterbyuser(this.value)" class="form-control" >
                                ...
                                <div class="d-flex flex-wrap justify-content-between mt-1">
                                    <div class="d-flex flex-column flex-grow-1 pr-8">
                                        <div class="d-flex flex-wrap mb-4">
                                            ...
                                            <i class="flaticon2-new-email mr-2 font-size-lg"></i>{{session()->get('user')->email}}m</a>
                                            <a href="#" class="text-dark-50 text-hover-primary font-weight-bold mr-lg-8 mr-5 mb-lg-0 mb-2">
...
@section('script')
  ...
  $(document).ready(async function () {
    ...
    await renderdata();
    await filterbyuser({{session()->get('user')->id }});

    google.charts.load('current', {'packages':['corechart']});
  });
  async function filterbyuser(id) {
    user=id;
    getTickets(id);
    CountTicketperBranch(id);
    CountTicketperMonth(id);
    CountTicketperIssue(id);
    getAllLogs(id);
  }
  function renderdata(){
...
//   filter the branch in the function
...
//   set value of month for the bar graph
  ...
  function filter_Classifications(records){
        ...
        for (const entry of data) {
            if (!seenNames.has(entry.class_name)) {
...
//   get the number of tickets per branch
  function CountTicketperBranch(uid){
    $(`#div_tickets`).html('');
    axios.get(`/api/count-tickets-per-department/0/${uid}`).then( response => {
...
//   get the number of tickets per month
  function CountTicketperMonth(uid){
    $(`#div_tickets`).html('');
    axios.get(`/api/count-tickets-per-month/0/${uid}`).then( response => {
      countpermonth = response.data;
      setvValueofGraph(department);
      filterbranchBu(response.data);
     ///set value of the charts

    }).catch(function (error) { console.log(error) });
  }
//   get the number of tickets per month
  function CountTicketperIssue(uid){
    $(`#div_tickets`).html('');
    axios.get(`/api/count-tickets-per-issue/0/${uid}`).then( response => {
    //   countpermonth = response.data;
     filter_Issue(response.data)

    }).catch(function (error) { console.log(error) });
  }
  function getTickets(uid){
    $(`#div_tickets`).html('');
    axios.get(`/api/get-all-tickets/0/${uid}`).then( response => {
       data = response.data;
       FilterData(response.data,uid);
...
// get cancel or disapproved tasks
  ...
  function FilterData(values,uid){
    ...
    percentage=(closed.length/(datas.length-discontinued.length))*100;
...
chart.draw(data, options);
    ...
    //  calculate logs
      ...
      function getAllLogs(uid){
            let startTimestamp="";
            let endTimestamp="";
            let total=0;
            let logs=[];

            axios.get(`/api/get-all-logs/0/${uid}`).then( response => {
