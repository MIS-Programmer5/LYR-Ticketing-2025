...
//   get the number of tickets per branch
  function CountTicketperBranch(){
    $(`#div_tickets`).html('');
    axios.get(`/api/count-tickets-per-department/0/{{session()->get('user')->id }}`).then( response => {
...
//   get the number of tickets per month
  function CountTicketperMonth(){
    $(`#div_tickets`).html('');
    axios.get(`/api/count-tickets-per-month/0/{{session()->get('user')->id }}`).then( response => {
...
//   get the number of tickets per month
  function CountTicketperIssue(){
    $(`#div_tickets`).html('');
    axios.get(`/api/count-tickets-per-issue/0/{{session()->get('user')->id }}`).then( response => {
    //   countpermonth = response.data;
  ...
  function getTickets(){
    $(`#div_tickets`).html('');
    axios.get(`/api/get-tickets/0/{{ session()->get('user')->id }}`).then( response => {
      data = response.data;
      discontinued=response.data.filter(function (item) {
...
// get cancel or disapproved tasks
  ...
  function getclosed(){
    let close={!!$ticket->getClosedTicket()!!};

    closed=close.filter(function (item) {
            return item.assignee_id === {{session()->get('user')->id}};
      });
...
chart.draw(data, options);
    ...
    //  calculate logs
      ...
      function getAllLogs(){
            ...
            axios.get(`/api/get-all-logs/{{session()->get('user')->id }}`).then( response => {
